...
describe('Testing linkKeywordsToFile function',()=>{
    ...
    it('Creates corresponding links when keywords exist', async()=>{
        const fileId = await idFactory()
        const id = await idFactory()
        const {keywords} = await fileFactory()

        jest.spyOn(keywordsRepository,'findByName').mockImplementation(():any=>({id}))
        jest.spyOn(keywordsRepository,'insert').mockImplementation(():any=>{})
        ...
        expect(keywordsRepository.insert).not.toBeCalled()
        expect(filesKeywordService.verifyFileKeywordLink).toBeCalled()
    })

    it('Creates corresponding links when keywords do not exist', async()=>{
        const fileId = await idFactory()
        const id = await idFactory()
        const {keywords} = await fileFactory()
